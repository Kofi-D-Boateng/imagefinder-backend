...
public class ImageFinder implements Finder {
    ...
    @GetMapping("/find")
    @Override
    public ResponseEntity<ConcurrentHashMap<String, ConcurrentHashMap<String, Set<String>>>> findImages(@RequestParam("url") String url, @RequestParam("mode")CrawlMode mode){
        System.out.println("url = " + url);
        System.out.println("mode = " + mode);
        RegexProcessor processor = new RegexProcessor("^(https?:\\/\\/)(www\\.)?([a-zA-Z0-9]+\\.)+[a-zA-Z]{2,}(\\/[a-zA-Z0-9-_]*)*\\/?$");
        Set<String> urlSet = processor.processStrings(url.split(","));
        if(urlSet.size() == 0){
    ...
    public ResponseEntity<byte[]> downloadZippedImages(@RequestParam("srcs") String commaSeparatedSource) throws IOException {

        List<String> srcs = new ArrayList<>(Arrays.asList(commaSeparatedSource.split(",")));
